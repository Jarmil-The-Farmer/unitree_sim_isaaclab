"""
Diffusion Action Provider for Isaac Lab
Integrates with the existing action provider system to run LeRobot diffusion policy with prerecorded images
"""
...
class DiffusionActionProvider(ActionProvider):
    """Action provider that runs LeRobot diffusion policy on prerecorded camera images."""
    
    def __init__(self, data_dir: str, episode_index: int, checkpoint_path: str, scale_factor: float = 1.0, playback_speed: float = 1.0):
        """
        Initialize diffusion action provider.
        
        Args:
            data_dir: Directory containing episodes data
            episode_index: Episode data to be inferred
        ...
        """
        super().__init__("DiffusionActionProvider")
        
        self.episode_path = Path(f"{data_dir}/episode_{episode_index:04d}")
        ...
        self.lerobot_available = True #ensure_lerobot_available()
        ...
        print(f"🎬 Diffusion Action Provider Initialized")
    ...
    def _load_diffusion_model(self):
        ...
        self.use_real_model = self.lerobot_available
    ...
    def get_action(self, env) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if self.time == 0.0:
                ...
                # Safely check for robot in scene
                robot = None
                if hasattr(env, 'scene'):
                    try:
                        ...
                        print(f"   Available entities: {list(env.scene._entities.keys()) if hasattr(env.scene, '_entities') else 'unknown'}")
                        return None
                else:
